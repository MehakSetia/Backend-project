...
export async function registerRoutes(app: Express): Promise<Server> {
  ...
  app.get("/api/admin/users", requireRole(["admin"]), async (req, res) => {
    try {
      ...
      // Convert each user to a plain object and remove password
  ...
  app.get("/api/hosts", isAuthenticated, async (req, res) => {
    try {
      ...
      res.json(hosts.map(host => host.toObject()));
    } catch (error) {
  ...
  app.get("/api/destinations", async (req, res) => {
    try {
      const destinations = await Destination.find(); // Assuming you have a Destination model
  ...
  app.get("/api/destinations/:id", async (req, res) => {
    try {
      // If your id is a string, use this:
      const destination = await Destination.findOne({ id: req.params.id });

      // If your id is a number, use this:
      // const destination = await Destination.findOne({ id: Number(req.params.id) });
